...
export const instructorService = {
  async getProfile(): Promise<ApiResponse<InstructorProfile>> {
    const response = await apiClient.get<ApiResponse<InstructorProfile>>("/users/me");
    return response.data;
  },

  async updateProfile(data: Partial<InstructorProfile>): Promise<ApiResponse<InstructorProfile>> {
    const response = await apiClient.put<ApiResponse<InstructorProfile>>("/users/me", data);
    return response.data;
  },
};
