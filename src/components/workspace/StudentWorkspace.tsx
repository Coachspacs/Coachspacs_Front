...
export function StudentWorkspace({ initialTab = "overview", onRoleSwitch }: StudentWorkspaceProps) {
  ...
  const [formData, setFormData] = useState({
    fullName: tStudent("defaultFullName"),
    ...
    videoSpeed: "1x",
    certificateName: tStudent("defaultCertificateName"),
    publicProfile: true,
              ...
              <div className="space-y-6 animate-in fade-in duration-150">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {courses
                    ...
                    .map((course) => (
                      <div key={course.id} className="rounded-3xl border border-slate-200/80 p-5 space-y-4 hover:shadow-md transition-all">
                        ...
                        <div className="space-y-1.5">
                          <div className="flex justify-between text-[11px] font-extrabold text-slate-500">
                            ...
                            <span className={course.isCompleted ? "text-emerald-700" : "text-[#0F5244]"}>
                              {course.progress}%
                            </span>
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() => setActiveTab("courses")}
                            className="flex-1 py-2.5 rounded-2xl bg-[#0F5244] hover:bg-[#07382E] text-white text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer"
                          >
                          ...
                          {course.isCompleted && (
                            <Link
                              href={`/${locale}/certificates/${course.certificateId || "CERT-892401"}`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {courses.filter((c) => c.isCompleted).map((cert) => (
                    <div key={cert.id} className="p-6 rounded-3xl border border-slate-200/80 space-y-4">
                      ...
                      <Link
                        href={`/${locale}/certificates/${cert.certificateId || "CERT-123"}`}
              ...
              <form onSubmit={handleSaveSettings} className="space-y-8 animate-in fade-in duration-150">
                ...
                <div className="flex justify-end">
                  <button
                    type="submit"
                    disabled={isSaving}
                    className="px-8 py-3 rounded-2xl bg-[#0F5244] hover:bg-[#07382E] text-white text-xs sm:text-sm font-extrabold shadow-sm active:scale-98 transition-all cursor-pointer disabled:opacity-70 flex items-center gap-2"
                  >
                    {isSaving ? (
                      <>
